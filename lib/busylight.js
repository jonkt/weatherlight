...
Busylight.prototype.pulse = function (colors, rate = 5000) {
    ...
    const refreshRate = 30; // ms between each animation step
    ...
    this.stepper = new Stepper(totalTicks, refreshRate, (index) => {
        ...
        this.send(true); // Bypass gamma for smoother animation steps
    });
};
 ...
 * @param {boolean} [skipGamma=false] If true, bypasses gamma correction.
 */
Busylight.prototype.send = function (skipGamma = false) {
    if (!this.connected || !this.device) return;

    const bufferToSend = [...this.buffer];

    // Apply color correction if enabled and not skipped
    if (this.degamma && !skipGamma) {
 ...
 * @returns {object|null} The device info object or null if not connected.
 */
Busylight.prototype.getDeviceInfo = function () {
    return this.connected ? this.deviceInfo : null;
};
