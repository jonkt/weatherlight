...
var BusyLight = /** @class */ (function () {
    ...
    BusyLight.prototype._connect = function () {
        var _this = this;
        try {
    ...
    BusyLight.prototype._keep_alive = function () {
        if (!this._device)
            return;
        if (!this._hid)
    ...
    BusyLight.prototype._checksumed = function (data) {
    ...
    BusyLight.prototype._send = function (data) {
        if (!this._hid)
            return;
        try {
    ...
    BusyLight.prototype._build_step = function (step, index) {
        if (!step)
            return false;
    ...
    BusyLight.prototype.state = function () {
        var s = '';
        var c;
    ...
    BusyLight.prototype._build_steps = function (steps) {
        var _this = this;
        this._steps = BusyLight.OFF.concat([]);
    ...
    BusyLight.prototype.program = function (steps) {
        if (this._once_timer)
    ...
    BusyLight.prototype.intesity = function (value) {
        if (value < 0)
            value = 0;
        if (value > 100)
            value = 100;
    ...
    BusyLight.prototype.blink = function (color, on, off) {
        if (!color)
            return false;
        if (color.charAt(0) === '#')
            color = color.substring(1);
    ...
    BusyLight.prototype.pulse = function (color) {
        if (!color)
            return false;
        if (color.charAt(0) === '#')
            color = color.substring(1);
    ...
    BusyLight.prototype.tone = function (tone, volume) {
        if (this._once_timer)
            clearTimeout(this._once_timer);
        this._once_timer = null;
        this._tone = tone;
    ...
    BusyLight.prototype.alert = function (tone, volume, color, blinkpulse, on, off) {
    ...
    BusyLight.prototype.once = function (tone, volume) {
        var _this = this;
        if (!this._device)
            return -1;
    ...
    BusyLight.prototype.tones = function () {
        if (!this._device)
            return [];
    ...
    BusyLight.prototype.durations = function () {
        if (!this._device)
            return [];
        if (!BusyLight.TONE_LEN[this._device.productId])
            return [];
    ...
    BusyLight.prototype.device = function () {
        if (!this._device)
            return '';
    ...
    BusyLight.prototype.is = function (device) {
        if (!device)
            return false;
        if (!this._device)
            return false;
    ...
    BusyLight.prototype.disconnect = function () {
        if (this._keepalive)
    ...
    BusyLight.prototype._b2s = function (data, f, t) {
    ...
    BusyLight.prototype.response = function () {
        var response = {};
    ...
    BusyLight.KEEPALIVE_SEC = 5;
    BusyLight.STEPS_COUNT = 7;
    BusyLight.VENDORID = 0x27BB;
    BusyLight.PRODUCTS = [0x3BCA, 0x3BCB, 0x3BCC, 0x3BCD, 0x3BCE, 0x3BCF];
    BusyLight.CMDS = {
        keepalive: 0x80,
        bootloader: 0x40,
        reset: 0x20,
