...
function Busylight(options) {
    ...
    this.degamma = true; // enable by default as per user feedback
...
Busylight.prototype.pulse = function(color, rate = 5000) {
    if (this.stepper) this.stepper.stop();

    const highColor = Array.isArray(color) ? color : parseColor(color);
    const lowColor = highColor.map(c => Math.round(c * 0.2)); // Dim for pulsing
    const colors = [highColor, lowColor];

    const refreshRate = 30; // ms per step
    const ticks = Math.floor(rate / refreshRate);

    this.stepper = new Stepper(ticks, refreshRate, (index) => {
        const colorIndex = Math.floor(index / (ticks / (colors.length-1) ));
        const start = colors[colorIndex];
        const end = colors[colorIndex + 1] || colors[0];

        const value = (index % (ticks/ (colors.length-1))) / (ticks / (colors.length-1));
